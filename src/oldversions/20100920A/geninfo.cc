...
void appendRecFSP(Particle p, std::vector<Particle> &children) {
	//std::cout << "Particle Decay mode = " << dynamic_cast<UserInfo&>(p.userInfo()).decayMode() << std::endl;
...
int appendGenFSP(const Gen_hepevt &gen, std::vector<Gen_hepevt> &children) {
	...
	for (int i = gen.daFirst(); i <= gen.daLast(); ++i) {
		...
		if(i==0) {
			std::cout << "[Zupanc] appendGenFSP: requesting Particle with ID = 0! Exiting." << std::endl;
			return 0;
		}
...
int commonMother(std::vector<int> &mothers) {
	...
	if(mothers.size()==0) {
		...
		//std::cout << "size of mothers vector == 1! Return mother = " << mothers[0] << std::endl;
		return mothers[0];
	}
...
void fillMothers(Particle &p, std::vector<int> &thisMothers, std::vector<int> &otherMothers) {
	...
	//std::cout << "This particle = " << p.child(0).genHepevt().idhep() << " with ID = " << p.child(0).genHepevt().get_ID() << std::endl;
	while(motherThis.mother()) {
		motherThis = motherThis.mother();
		//std::cout << " -> mother = " << motherThis.idhep() << " ID = " << motherThis.get_ID() << std::endl;
		thisMothers.push_back(motherThis.get_ID());
	}

	for(int i = 1; i<(int)p.nChildren(); ++i) {
		motherThis = p.child(i).genHepevt();
		//std::cout << "Other particle (" << i << ") = " << motherThis.idhep() << " with ID = " << motherThis.get_ID() << std::endl;
		while(motherThis.mother()) {
			motherThis = motherThis.mother();
			//std::cout << " -> mother = " << motherThis.idhep() << " ID = " << motherThis.get_ID() << std::endl;
...
int getCommonMother(Particle &A, Particle &B) {
	...
	if(!motherID) {
		std::cout << "[Zupanc] setMCtruth: requesting particle with ID = 0! Exiting." << std::endl;
		return 0;
	}
...
int findCommonMother(Gen_hepevt pThis, Gen_hepevt pOther, int level) {
	...
	//std::cout << "This = " << pThis.idhep() << " [ID = " << pThis.get_ID() << "] vs. " << "Other = " << pOther.idhep() << " [ID = " << pOther.get_ID() << std::endl;
	int i = 1;
	while(motherThis.mother()) {
		motherThis = motherThis.mother();
		//std::cout << "[" << i << "] This mother = " << motherThis.idhep() << " ID = " << motherThis.get_ID() << std::endl;
		i++;

		if(i>level) {
			//std::cout << "Other: " << pOther.idhep() << " [ID = " << pOther.get_ID() << std::endl;

			Gen_hepevt motherOther(pOther);
			while(motherOther.mother()) {
				motherOther = motherOther.mother();
				//std::cout << "[" << i << "] Other mother = " << motherOther.idhep() << " ID = " << motherOther.get_ID() << std::endl;
	...
	if(reconstructed.size() == generated.size()) {
		...
		for (int i = 0; i < (int)reconstructed.size(); ++i) {
			if(reconstructed[i].genHepevt()) {
				...
				for (int j = 0; j < (int)generated.size(); ++j) {
					if(reconstructed[i].genHepevt().get_ID()==generated[j].get_ID()) {
						...
						if(reconstructed[i].lund()!=generated[j].idhep())
							missID++;
						if(reconstructed[i].lund()==111 && dynamic_cast<UserInfo&>(reconstructed[i].userInfo()).genHepevtLink()==5) {
							missPi0++;
						}
						break;
					}
				...
				if(!link) {
					std::cout << "[Zupanc] compareFinalStates: Particle (" << reconstructed[i].lund() << " with hepevt = " << reconstructed[i].genHepevt().idhep() << ") not found in list of FSP (gen)!" << std::endl;
					return -11;
				}
			} else {
				std::cout << "[Zupanc] compareFinalStates: Particle without link to genHepevt! [-10]" << std::endl;
				return -10;
			}
		}
		if(missID && missPi0)
		...
		for (int i = 0; i < (int)generated.size(); ++i) {
			...
			for (int j = 0; j < (int)reconstructed.size(); ++j) {
				if(reconstructed[j].genHepevt()) {
					...
					std::cout << "[Zupanc] compareFinalStates: Particle without link to genHepevt! [-9]" << std::endl;
					return -9;
				}
			}
			if(!link) {
		...
		if(missing) {
			...
			for (int i = 0; i < (int)missP.size(); ++i) {
				if(generated[missP[i]].idhep()!=22 && abs(generated[missP[i]].idhep())!=12 && abs(generated[missP[i]].idhep())!=14 && abs(generated[missP[i]].idhep())!=16)
					masslessOnly = 0;
			}
			if(masslessOnly) {
				return 4;
			...
			std::cout << "[Zupanc] compareFinalStates: At least one particle should be missing!" << std::endl;
			return -8;
		}
	} else {
		std::cout << "[Zupanc] compareFinalStates: More reconstructed than generated particle in final state!" << std::endl;
		return -5;
	}
}
...
void setMCtruthPi0(Particle &p) {
	...
	for(int i=0; i<(int)p.nChildren(); ++i) {
		...
		if(p.child(i).mdstGamma()) {
			...
			if (hep){
				...
				//std::cout << " link found (" << dynamic_cast<UserInfo&>(p.userInfo()).genHepevtLink() << ") " << std::endl;
			} else {
				std::cout << " [Zupanc] setMCtruthPi0: child " << i << " has no link! " << std::endl;
			}
		} else {
			std::cout << " [Zupanc] setMCtruthPi0: pi0 child is not mdstGamma! Exit. " << std::endl;
		}
	}
	...
	if(p.child(0).genHepevt() && p.child(1).genHepevt()) {
		...
		if(mother1.mother() && mother2.mother()) {
			if(mother1.mother().get_ID()==mother2.mother().get_ID()) {
				if(mother1.mother().get_ID()==0) {
					std::cout << "[Zupanc] setMCtruthPi0: requesting particle with ID = 0! Exiting." << std::endl;
					return;
				}
				...
				//std::cout << "[Zupanc] setMCtruthPi0: pi0 different mothers " << mother1.mother().idhep() << " vs " << mother2.mother().idhep() << std::endl;
			...
			//std::cout << "[Zupanc] setMCtruthPi0: one child without mother " << std::endl;
			if(mother1.mother()) {
				if(mother1.mother().get_ID()==0) {
					std::cout << "[Zupanc] setMCtruthPi0: requesting particle with ID = 0! Exiting." << std::endl;
					return;
		...
		std::cout << "[Zupanc] setMCtruthPi0: Only first photon with link to gen_hepevt: " << std::endl;
		...
		std::cout << "[Zupanc] setMCtruthPi0: Only second photon with link to gen_hepevt: " << std::endl;
		...
		std::cout << "[Zupanc] setMCtruthPi0: Neither of two photons has link to gen_hepevt!" << std::endl;
	}
...
void setMCtruth(Particle &p){
	...
	//std::cout << " Particle = " << p.lund() << " with " << p.nChildren() << " children." << std::endl;
	//std::cout << " Status before: Checked = " << dynamic_cast<UserInfo&>(p.userInfo()).genHepevtChecked() << " with Link = " << dynamic_cast<UserInfo&>(p.userInfo()).genHepevtLink() << std::endl;
	...
	if(p.lund()==111) {
		...
		setMCtruthPi0(p);
		//std::cout << " Pi0 link (" << dynamic_cast<UserInfo&>(p.userInfo()).genHepevtLink() << ") " << std::endl;
		return;
	}
	...
	//std::cout << " Particle != FSP : nChidlren = " << p.relation().nChildren() << std::endl;
	for(int i=0; i<nChildren; ++i) {
		//std::cout << " [" << i << "]" << "lund = " << p.relation().child(i).lund() << " checked/link = " << dynamic_cast<UserInfo&>(p.child(i).userInfo()).genHepevtChecked()	<< "/" << dynamic_cast<UserInfo&>(p.child(i).userInfo()).genHepevtLink() << std::endl;
	...
	//std::cout << "Checking for clones: " << std::endl;
	// check that there are no clones
	for(int i=0; i<nChildren; ++i) {
		for(int j=i+1; j<nChildren; ++j) {
			//std::cout << "[" << i << "/" << j << "] = " << p.relation().child(i).genHepevt().get_ID() << " ?? " << p.relation().child(j).genHepevt().get_ID() << std::endl;
	...
	/*
	//std::cout << "Searching for common Mother: " << std::endl;
	const Gen_hepevt & motherThis(p.child(0).genHepevt());
	int level = 0;
	int motherID = 0;
	//std::cout << "Base particle: " << p.child(0).lund() << " genHepevt = " <<  p.child(0).genHepevt().idhep() << " ID = " << p.child(0).genHepevt().get_ID() << std::endl;
	while(motherThis.mother()) {
		int ID = 0;
		level++;
		for(int i = 1; i<nChildren; ++i) {
			//std::cout << " [" << i << "]" << p.child(i).lund() << " genHepevt = " <<  p.child(i).genHepevt().idhep() << " ID = " << p.child(i).genHepevt().get_ID() << std::endl;
			ID = findCommonMother(p.child(0).genHepevt(), p.child(i).genHepevt(), level);
			//std::cout << " ID of commo mother = " << ID << std::endl;
			if(!ID) {
				//std::cout << "No common mother! Exit." << std::endl;
				return;
			}
			commonMothers.push_back(ID);
		}
		//std::cout << "Common mother of all at level " << level << std::endl;
		motherID = commonMother(commonMothers);
		if(motherID) {
			//std::cout << "Common mother found with ID = " << motherID << std::endl;
			break;
		}
	...
	Gen_hepevt_Manager& GenMgr = Gen_hepevt_Manager::get_manager();
	const Gen_hepevt& thisGen = GenMgr(Panther_ID(motherID));
	//std::cout << "Common mother: ID = " << motherID << " idhep = " << thisGen.idhep() << std::endl;
	...
	// ID = 3; FSP have common mother, but at least one massive particle is missing
	// ID = 4; FSP have common mother, but at least one massless particel is missing (FSR photon or neutrino)
	// ID = 5; final state includes pi0, which has only one daughter gamma with link to gen_hepevt
	// ID = 6; ID = 2 and 5 are true
	// ID = -1; common mother is virtual gamma (10022) or Upsilon(4S,5S) (300553,9000553)
	...
	/*if(truth==-5) {
		std::cout << " Mother = " << thisGen.idhep() << " with ID = " << thisGen.get_ID() << std::endl;
		std::cout << "Reconstructed: " << std::endl;
		for (int i = 0; i < (int)reconstructed.size(); ++i) {
			std::cout << i << ": lund = " << reconstructed[i].lund() << std::endl;
			if(reconstructed[i].genHepevt()) {
				std::cout << " -> hepevt = " << reconstructed[i].genHepevt().idhep()  << " with Mother ID = " << reconstructed[i].genHepevt().mother_ID()<< std::endl;
			} else {
				std::cout << " -> without link to genhepevt " << std::endl;
			}
		}
		std::cout << "Generated: " << std::endl;
		for (int i = 0; i < (int)generated.size(); ++i) {
			std::cout << i << ": idhep = " << generated[i].idhep() << std::endl;
		}
		dumpGenFSP(thisGen);
	}*/
