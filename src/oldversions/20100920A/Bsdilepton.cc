...
void Bsdilepton::event(BelleEvent* evptr, int* status)
{
  ...
  ++numberOfEvents;
  int eventNumber = numberOfEvents;
  ...
  for ( std::vector< Mdst_charged >::const_iterator i = chg_mgr.begin();
    ...
    // Create an alias "isSigMC" for the MC information about chg to be used in
    //   if() statements.
  ...
  */    
  ...
  for ( std::vector< Particle >::iterator j = leptonList.begin();
    ...
    for ( std::vector< Particle >::iterator i = j;
      ...
      if ( lepton0MomentumCm.vect().mag() < cuts.minLeptonMomentumCm ||
           lepton0MomentumCm.vect().mag() > cuts.maxLeptonMomentumCm ) {
        continue;
      }

      if ( lepton1MomentumCm.vect().mag() < cuts.minLeptonMomentumCm ||
           lepton1MomentumCm.vect().mag() > cuts.maxLeptonMomentumCm ) {
        continue;
      }
      
      // Store the MC truth to the lepton candidates.
      setMCtruth( lepton0 );
      setMCtruth( lepton1 );

      // Make an event candidate.
      DileptonEvent eventCandidate( lepton0, lepton1 );
